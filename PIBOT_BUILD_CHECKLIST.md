...
Press Enter to accept `pibot.local`, or type the IP if you have it. It copies:
...
Open `http://pibot.local:5000` in a browser.
...
On the Mac: Finder → Go → Connect to Server → `smb://pibot.local/Ohbot`
...
called `bootfs` appears. Open **PIBOT_IP.txt** on it. That's the address it had
...
### Why `pibot.local` fails on venue networks

The `.local` name works by shouting "who is pibot?" to everyone on the network
...
for security, partly to stop people snooping. On such a network `pibot.local`
